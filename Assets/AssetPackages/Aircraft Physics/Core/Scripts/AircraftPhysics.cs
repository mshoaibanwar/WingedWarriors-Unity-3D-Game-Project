...
public class AircraftPhysics : MonoBehaviour
{
    ...
    public AirplaneController apController;
    ...
    public void Brake(bool isBraking) //increases drag on wheels
    {
        ...
        if (isBraking)
        {
            ...
            friction = 0.1f;
            SetThrustPercent(0.2f);
            apController.SetThrust(0.2f);
            rb.drag = drag;
            if(velocity.z < 4f && velocity.z > -4f)
            {
                SetThrustPercent(0f);
                apController.SetThrust(0f);
            }
        }
        else
        {
            friction = 0f;
            // SetThrustPercent(thrust);
            // apController.SetThrust(thrust);
             rb.drag = 0f;
        }
